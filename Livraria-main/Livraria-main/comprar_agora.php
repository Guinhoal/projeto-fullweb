...
$stmt = $conexao->prepare("SELECT * FROM produto WHERE idproduto = :produto_id");
...
$stmt = $conexao->prepare("SELECT quantidade FROM estoque WHERE produto_idproduto = :produto_id");
$stmt->bindParam(':produto_id', $produto_id, PDO::PARAM_INT);
$stmt->execute();
$estoque = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$estoque || $estoque['quantidade'] < 1) {
    echo "<script>alert('Produto fora de estoque.'); window.location.href='index.php';</script>";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($etapa === 1) {
        ...
        header("Location: comprar_agora.php?etapa=2&produto_id=$produto_id");
        exit();
    } elseif ($etapa === 2) {
        $forma_pagamento = $_POST['forma_pagamento'];
        $_SESSION['forma_pagamento'] = $forma_pagamento;
        header("Location: comprar_agora.php?etapa=3&produto_id=$produto_id");
        exit();
    } elseif ($etapa === 3) {
        ...
        if ($forma_pagamento === 'credito' && $creditos < $total_creditos) {
            echo "<script>alert('Você não tem créditos suficientes para esta compra.'); window.location.href='comprar_agora.php?etapa=2&produto_id=$produto_id';</script>";
            exit();
        }
        ...
        $stmt->bindValue(':produto_idproduto', $produto['idproduto'], PDO::PARAM_INT);
        $stmt->bindValue(':quantidade', 1, PDO::PARAM_INT); 
        $stmt->bindValue(':preco_unitario', $produto['preco']);
        $stmt->execute();

        $stmt = $conexao->prepare("UPDATE estoque SET quantidade = quantidade - 1 WHERE produto_idproduto = :produto_id");
        $stmt->bindParam(':produto_id', $produto_id, PDO::PARAM_INT);
        $stmt->execute();
...
<head>
    <meta charset="UTF-8">
    <title>Comprar Agora - GhostBooks</title>
